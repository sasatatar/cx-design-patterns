 ...
 * we define only the value binding, and the field loads the options in its private store.
...
export const lookupFieldGenerator = (configurator) => createFunctionalComponent(({visible=true, ...props}) => {
    ...
    let {
        ...
        viewMode,
        tooltip,
        textTpl, // Template that will use search field's internal store bindings to display user friendly selection text
        ...rest
    } = config;
